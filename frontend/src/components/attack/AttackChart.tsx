...
import { attackApi, findingsApi } from '../../services/api'
...
export default function AttackChart() {
  ...
  const totalTechniques = techniquesData.length
  const totalDetections = techniquesData.reduce((sum, t) => sum + t.count, 0)
  const totalCritical = techniquesData.reduce((sum, t) => sum + (t.severities?.critical || 0), 0)
  const totalHigh = techniquesData.reduce((sum, t) => sum + (t.severities?.high || 0), 0)

  const severityColors = ['#d32f2f', '#f57c00', '#1976d2', '#388e3c']
                ...
                <PieChart>
                  <Pie
                    data={[
                      ...
                      { name: 'Medium', value: techniquesData.reduce((sum, t) => sum + (t.severities?.medium || 0), 0) },
                      { name: 'Low', value: techniquesData.reduce((sum, t) => sum + (t.severities?.low || 0), 0) },
                    ].filter(d => d.value > 0)}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
