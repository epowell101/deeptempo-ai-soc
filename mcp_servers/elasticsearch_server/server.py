...
def get_elasticsearch_config():
    """Get Elasticsearch configuration from integrations config."""
    try:
        from ui.integrations_config import IntegrationsConfigDialog
        config = IntegrationsConfigDialog.get_integration_config('elasticsearch')
        return config
    except Exception as e:
        logger.error(f"Error loading Elasticsearch config: {e}")
        return {}

server = Server("elasticsearch-server")
...
async def handle_call_tool(
    ...
    if not config:
        return [types.TextContent(
            ...
            text=json.dumps({
                ...
                "message": "Please configure Elasticsearch in Settings > Integrations"
            }, indent=2)
        )]
    ...
    try:
        ...
        if name == "es_get_document":
            # TODO: Implement es_get_document
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "Not implemented",
        ...
        if name == "es_bulk_query":
            # TODO: Implement es_bulk_query
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "Not implemented",
        ...
        if name == "es_create_index":
            # TODO: Implement es_create_index
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "Not implemented",
        ...
        logger.error(f"Error in Elasticsearch tool {name}: {e}")
        return [types.TextContent(
            type="text",
            text=json.dumps({
                "error": str(e),
