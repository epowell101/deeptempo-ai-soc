...
def get_postgresql_config():
    """Get PostgreSQL configuration from integrations config."""
    try:
        from ui.integrations_config import IntegrationsConfigDialog
        config = IntegrationsConfigDialog.get_integration_config('postgresql')
        return config
...
async def handle_call_tool(
    ...
    if not config:
        return [types.TextContent(
            ...
            text=json.dumps({
                ...
                "message": "Please configure PostgreSQL in Settings > Integrations"
            }, indent=2)
        )]
    ...
    try:
        ...
        if name == "pg_create_table":
            # TODO: Implement pg_create_table
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "Not implemented",
        ...
        if name == "pg_execute_procedure":
            # TODO: Implement pg_execute_procedure
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "Not implemented",
        ...
        logger.error(f"Error in PostgreSQL tool {name}: {e}")
        return [types.TextContent(
            type="text",
            text=json.dumps({
                "error": str(e),
