"""
Data loader for Streamlit app - loads findings from the AI SOC data files.
"""
...
def load_findings(data_dir: Path = None) -> dict:
    """Load findings from the data directory."""
    if data_dir is None:
...
def load_cases(data_dir: Path = None) -> list:
    """Load cases from the data directory."""
    if data_dir is None:
        data_dir = Path(__file__).parent.parent / "data"
...
def findings_to_attack_data(findings: list) -> dict:
    ...
    for finding in findings:
        ...
        if entity.get("src_ip", "").startswith("10.") or entity.get("src_ip", "").startswith("192.168."):
            pass  # Internal IP
        elif entity.get("dst_ip"):
    ...
    for technique, stats in technique_counts.items():
        ...
        mitre_techniques.append({
            ...
            "name": name,
            "tactic": tactic,
            "confidence": round(stats["total_confidence"] / stats["count"], 3),
            "count": stats["count"]
        })
    ...
    if not phases:
        phases = [
            {"phase": 1, "name": "Detection", "time": "00:00", "description": "Anomalies detected", "techniques": [], "entities": list(hosts)[:3]}
        ]
