        ...
        try:
            ...
            if include_findings_detail and findings:
                ...
                for i, finding in enumerate(findings[:50], 1):  # Limit to first 50 findings
                    ...
                    # MITRE techniques
                    mitre_preds = finding.get('mitre_predictions', {})
                    if mitre_preds:
                        top_techniques = sorted(mitre_preds.items(), key=lambda x: x[1], reverse=True)[:5]
                        tech_str = ", ".join([f"{t[0]} ({t[1]:.2f})" for t in top_techniques])
        ...
        try:
            ...
            if findings:
                ...
                for finding in findings:
                    ...
                    anomaly_score = f"{finding.get('anomaly_score', 0):.3f}"
                    
                    # Get top MITRE techniques
                    mitre_preds = finding.get('mitre_predictions', {})
                    if mitre_preds:
                        top_techs = sorted(mitre_preds.items(), key=lambda x: x[1], reverse=True)[:3]
                ...
                for i, finding in enumerate(findings, 1):
                    ...
                    mitre_preds = finding.get('mitre_predictions', {})
                    if mitre_preds:
                        for tech_id, confidence in sorted(mitre_preds.items(), key=lambda x: x[1], reverse=True)[:10]:
                            finding_info.append([f"MITRE: {tech_id}", f"{confidence:.3f}"])
